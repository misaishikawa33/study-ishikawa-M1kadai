...
・スケールパラメータ
実行：
python experiment_gauss_newton_method.py {画像のパス} {真値のスケール} {真値の角度(deg)} --scale_init {初期値のスケール} --theta_init {初期値の角度(deg)} --threshold {収束判定の閾値} --max_loop {最大反復回数} --kernel_size {ガウシアンフィルタのカーネルサイズ} --sigma {ガウシアンフィルタのシグマ} --output_path {実験結果の出力先のフォルダパス}
