...
def rms_error(u_true, u_est_list):
    ...
    norms = [norm(err)**2 for err in errors]
...
for sigma in sigma_values:
    ...
    for _ in range(num_trials):
        noise = np.random.normal(0, sigma, X_true.shape)  # ガウスノイズ
        X_noisy = X_true + noise                          # ノイズ付加

        # --- 最小二乗法で推定 ---
        u_ls = estimate_least_squares(X_noisy)
        if np.dot(u_ls, u_true) < 0:
            u_ls = -u_ls  # 符号合わせ（方向の一致）
    ...
    print(f"σ={sigma:.2f}, LS RMS={rms_ls:.6f}, ML RMS={rms_ml:.6f}")

# --- 結果をグラフに描画 ---
plt.figure(figsize=(8, 5))
plt.plot(sigma_values, np.array(rms_ls_list)*1e4, marker='o', label='LS')  # LS誤差を10^-4スケールで表示
plt.plot(sigma_values, np.array(rms_ml_list)*1e4, marker='x', label='ML')  # ML誤差
plt.xlabel("σ")
plt.ylabel("RMS × $10^{-4}$")  # 軸ラベル
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
