...
def kcr_lower_bound(X, u_true, sigma2):
    ...
    for x, y in X:
        ...
        denom = max(u_true @ V @ u_true, 1e-12)  # 安定化
        M += np.outer(xi, xi) / denom

    eigvals = np.linalg.eigvalsh(M)
    eigvals = eigvals[eigvals > 1e-6]  # カットオフ設定（鋭いKCRに）
    if len(eigvals) < 1:
        return 1e10  # 安全策：誤差が大きすぎるとき
...
for sigma in sigma_values:
    ...
    rms_kcr = kcr_lower_bound(X_true, u_true, sigma**2)
...
# plt.plot(sigma_values, np.array(rms_kcr_list)*1e4, linestyle='--', color='green', label='KCR')  # コメントアウト
