...
plt.plot(sigma_values, np.array(rms_list) * 1e4, marker='o')  # スケールを1e4倍して表示
plt.xlabel("σ (ノイズの標準偏差)")
plt.ylabel("RMS誤差 × $10^{-4}$")  # ラベルにスケーリング明記
plt.title("ノイズレベルに対するRMS誤差（スケール調整）")
plt.grid(True)
plt.tight_layout()
plt.show()
